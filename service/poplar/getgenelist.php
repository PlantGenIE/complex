...
//$ip=$uuid;
function getdefaultsamplelist_pt()
{
    include("koneksi.php");
    $ip            = $pt_uuid;
...
function getdefaultgenelist_pt()
{
    include("koneksi.php");
    $ip         = $pt_uuid;
...
function getdefaultgenelistname_pt()
{
    include("koneksi.php");
    $ip         = $pt_uuid;
...
function updategenebasket_pt($genearray)
{
    global $config;
	$dp_pt=mysql_connect(
	...
	mysql_select_db("v4genebaskets",$dp_pt) or die("database tidak ada"); 
	...
	if(!isset($_COOKIE['popgenie_uuid']))
	{
	...
	setcookie ("popgenie_uuid", $popuuids, $expire, '/', $setDomain, 0 );
	}else{
	$popuuid=$_COOKIE['popgenie_uuid'];
	}
    ...
    $check                   = mysql_query("select * from defaultgenebaskets where ip='$ip'",$dp_pt);
    if (mysql_num_rows($check) == 0) {
        // NO DEFAULT GENEBASKETS,INSTERED
        $initcounts = count($genessendaddStringArray);
        mysql_query("insert into genebaskets(gene_basket_id,gene_basket_name,harga,genelist,ip) values('$kode','default','$initcounts','$genessendaddString','$ip')",$dp_pt) or die("data gagal di insert");
        mysql_query("insert into defaultgenebaskets(defaultgenebaskets.gene_basket_id,defaultgenebaskets.ip) SELECT LAST_INSERT_ID(gene_basket_id),'$ip' from genebaskets WHERE ip='$ip' ORDER BY gene_basket_id DESC Limit 1;",$dp_pt);
        ...
        $defaultresults = mysql_query($defaultstr,$dp_pt) or die("query gagal dijalankan");
        if (mysql_num_rows($defaultresults) != 0) {
            $defaultgeenedata = mysql_fetch_assoc($defaultresults);
            ...
            if (strlen($dbgenesStr) < 5) {
                ...
                mysql_query("update genebaskets set genelist='$genessendaddString',harga='$initcount' where gene_basket_id='$basketid'",$dp_pt) or die("data gagal di update");
                ...
                mysql_query("update genebaskets set genelist='$updategenelist',harga='$updatecount' where gene_basket_id='$basketid'",$dp_pt) or die("data gagal di update");
...
function removegenebasket_pt($removegenearray){
    include("koneksi.php"); 
    $ip                      = $pt_uuid;
...
function updategenebasketall_pt($updateallarr, $name = FALSE)
{
    global $config;
	if($name){
		$basketname=$name;
...
function sharetable_pt($updateallarr, $name = FALSE, $randid = FALSE)
{
    if ($name) {
        $basketname = $name;
    } else {
...
function checksharedlinkexist_pt($randid)
{
    include("koneksi.php");
    $ip      = $pt_uuid;
    $checkme = mysql_query("select * from share_table where randid='$randid';");
...
function removegobasket_pt($removegenearray)
{
